...
> **Reusing an existing resource group?** If `AZURE_RESOURCE_GROUP` points to an RG that was previously provisioned (by you, a teammate, CI, or an earlier env), expect `RoleAssignmentUpdateNotPermitted` on the developer-convenience role assignments. **This is almost always benign** — the role is already correctly granted to the current principal; ARM just refuses the no-op re-create. Do **not** start an RBAC investigation; jump straight to the decision tree in the [error row below](#common-failure-modes----hosted) and verify with `azd ai project show` first. The two most common preventatives: use a fresh env name (`azd env new <name>`) so a fresh RG is created, or accept the error and move on.
...
| `RoleAssignmentUpdateNotPermitted` during provision | **Almost always benign when reusing an existing RG — do NOT investigate before completing the 3 checks below (each runs in seconds).** The error fires when ARM tries to *re-create* an existing role assignment with the same deterministic GUID; if the existing assignment already grants the same role to the same principal, the role is effectively in place and `azd deploy` will succeed. **Decision tree:** (1) `azd ai project show --output json` — if it returns the project, the Foundry resources were created (ARM does not roll back peers); **continue to `azd deploy`**, stop investigating. (2) If `azd deploy` later 403s on the Foundry data plane, confirm the failing assignment is yours: pull the GUID from `error.details[].target` in the provision output, then `az role assignment list --scope <scope-from-error> --query "[?name=='<guid>'].{principal:principalId,role:roleDefinitionName}" -o jsonc` and compare `principalId` to `az ad signed-in-user show --query id -o tsv`. If they match → role is correctly assigned, the 403 is a different problem (check the data-plane error). (3) Only if `principalId` does **not** match (RG owned by a different principal, or principal was rotated) → repair: `az role assignment delete --ids <id-from-target>` then `azd provision --no-prompt`, or grant directly with a fresh name: `az role assignment create --assignee <your-oid> --role "Azure AI User" --scope <project-id>` (project id is `AZURE_AI_PROJECT_ID` in `azd env get-values`). **Never** run `az role assignment list --scope <rg>` without the `?name==` filter — it returns dozens of rows and sends the model down a rabbit hole. |
