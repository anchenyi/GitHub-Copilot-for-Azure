...
14. **Offer option to visualize data in data wrangler.** When referencing a dataset file or evaluation result file (in `.foundry/datasets/`, `.foundry/results/`), offer the user the option to visualize the data in data wrangler. If the user confirms, run CLI `data-wrangler <file-name>` to launch a browser to visualize the data.
